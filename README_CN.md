...
- **Range 下载安全性增强**：模型下载的每个分块现在必须收到 `206 Partial Content` 响应。对于忽略 Range 并返回 `200 OK`，或返回 `400 Bad Request` 拒绝 Range 的服务器，程序会自动回退到单线程下载，避免生成损坏文件。
- **发布前清理**：重新构建发布前清理本地构建目录、分发目录、Python 缓存和临时下载产物；这些生成文件仍会被版本控制排除。

## v1.4.4 更新内容

- **模型下载 Range 回退**：修复模型下载在 Cloudflare R2 / S3 预签名 URL 上返回 `400 Bad Request` 失败的问题。当服务器拒绝 Range 请求（400）时，下载器现在会自动回退到单线程下载，而不是失败。

## v1.4.3 更新内容

- **视频 CDN 503 回退**：当视频缩放 URL 返回 `503 Service Unavailable` 时，下载器会自动切换到原始视频 URL 重试，避免反复请求不可用的缩放地址。
